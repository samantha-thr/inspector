...
def score_pair(a, b, links_a, links_b) -> tuple[int, int, int, int, str, str]:
    ...
    if string: reasons.append("same string fingerprint")
...
def rebuild_evidence(callback: Callable[[dict], None] | None = None, max_family_size: int = 75) -> dict:
    ...
    families = db.families(100000); pairs = added = 0; db.begin()
...
def texture_pair_score(a, b):
    ...
    if exact: reasons.append("exact texture SHA")
    hdist = hex_hamming(a["ahash"], b["ahash"])
    perceptual = max(0, round(100 * (1 - hdist / 64))) if a["ahash"] and b["ahash"] else 0
    if perceptual >= 95: reasons.append("near-identical perceptual hash")
    elif perceptual >= 85: reasons.append("similar perceptual hash")
    histogram = 100 if a["histogram_hash"] and a["histogram_hash"] == b["histogram_hash"] else 0
    if histogram: reasons.append("same color histogram")
    ...
    if color >= 95: reasons.append("same average color")
    elif color >= 85: reasons.append("similar average color")
    alpha_diff = abs(float(a["alpha_coverage"] or 0) - float(b["alpha_coverage"] or 0))
    alpha = max(0, round(100 - alpha_diff * 100))
    if alpha >= 98 and (a["has_alpha"] or b["has_alpha"]): reasons.append("same alpha coverage")
    fmt = 100 if (a["dds_format"] and a["dds_format"] == b["dds_format"]) else 0
    if fmt: reasons.append("same DDS format")
    ...
    if aw and ah and bw and bh:
        if aw == bw and ah == bh: size_score = 100; reasons.append("same resolution")
        elif abs((aw / ah) - (bw / bh)) < 0.02: size_score = 70; reasons.append("same aspect ratio")
    if exact:
        overall = 100; etype = "identical_texture"
    else:
        ...
        if histogram >= 100 and perceptual < 95: etype = "likely_recolor_or_adjustment"; reasons.append("histogram match with perceptual difference")
        elif perceptual >= 92 and color >= 90: etype = "near_identical_texture"
        elif perceptual >= 80 and (brightness_delta > .08 or saturation_delta > .08): etype = "likely_brightness_or_saturation_variant"; reasons.append("brightness/saturation differs")
        elif overall >= 75: etype = "likely_derived_texture"
        elif overall >= 55: etype = "possible_texture_relationship"
        else: etype = "weak"
...
def rebuild_texture_evidence(callback: Callable[[dict], None] | None = None, max_group_size: int = 200) -> dict:
    ...
    This does not require texture families to be rebuilt first. It pulls candidate
    groups directly from sha256, ahash, and histogram_hash.
    """
    db = Database(); started = time.time(); db.clear_texture_evidence()
    sql = db.db
    groups = []
    for method, expr in [("exact_sha256", "sha256"), ("perceptual_ahash", "ahash"), ("color_histogram", "histogram_hash")]:
        rows = sql.execute(f"""
        ...
        """).fetchall()
        for row in rows:
            groups.append({"method": method, "expr": expr, "key_value": row["key_value"], "count": row["count"]})
    checked = added = 0
    seen_pairs = set()
    db.begin()
    for gi, group in enumerate(groups, 1):
        members = sql.execute(f"SELECT * FROM textures WHERE {group['expr']}=? ORDER BY folder,filename LIMIT ?", (group["key_value"], max_group_size)).fetchall()
        for i in range(len(members)):
            for j in range(i + 1, len(members)):
                a, b = members[i], members[j]
    ...
    db.commit(); db.close()
    return {"candidate_groups": len(groups), "pairs_checked": checked, "texture_evidence_pairs": added, "elapsed": time.time() - started}
