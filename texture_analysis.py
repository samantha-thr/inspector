...
def parse_dds(path: Path) -> dict[str, Any]:
    result = {
        ...
        "dds_fourcc": "", "dds_format": "", "dds_rgb_bits": 0,
        "dds_has_alpha": 0, "dds_is_cubemap": 0, "dds_is_volume": 0,
        "dds_estimated_vram": 0, "dds_header_status": "not_dds",
    }
    try:
    ...
    try:
        ...
        pf_size, pf_flags, fourcc_raw, rgb_bits, rmask, gmask, bmask, amask = struct.unpack_from("<II4sIIIII", header, 76)
        caps, caps2, caps3, caps4, reserved2 = struct.unpack_from("<5I", header, 104)
        fourcc = fourcc_raw.decode("ascii", errors="ignore").strip("\x00 ").strip()
        fmt, block_size = DDS_FORMATS.get(fourcc, ("", 0))
        if not fmt:
            if pf_flags & 0x40:
                fmt = f"Uncompressed RGB/RGBA {rgb_bits}-bit"
            elif fourcc:
                fmt = f"FOURCC {fourcc}"
            else:
                fmt = "Unknown DDS pixel format"
...
def analyze_texture(path: Path) -> dict[str, Any]:
    ...
    result = {
        ...
        "avg_a": 255.0, "ahash": "", "analysis_status": "hash_only",
        **dds,
    }
