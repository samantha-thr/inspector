...
def timestamped_report_path(prefix: str) -> Path:
    folder = REPORTS_PATH / "evidence"
    folder.mkdir(parents=True, exist_ok=True)
    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    return folder / f"{prefix}_{stamp}.csv"
...
def quick_status():
    ...
    console.print(
        ...
        f"{rel['texture_evidence_pairs']:,} texture evidence"
    )
    db.close()
...
def main_menu():
    while True:
        ...
        items = [
            "Scan Manager", "Research / Analysis", "Search Models", "Search Textures",
            "Model Explorer", "Texture Explorer", "Duplicates", "Families",
            "Model Evidence Browser", "Texture Evidence Browser", "Statistics", "Exit",
        ]
...
def progress_runner(title, func, *args, **kwargs):
    ...
    with Progress(
        ...
        def cb(info):
            ...
            progress.update(
                task, total=total, completed=info.get("index", 0), description=desc,
...
def research_menu():
    while True:
        console.clear(); header(); quick_status()
        opts = [
            "Rebuild Model ↔ Texture Links", "Rebuild Model Families", "Rebuild Texture Families",
            "Rebuild Model Evidence Pairs", "Rebuild Texture Evidence Pairs",
            "Export Model Evidence CSV", "Export Texture Evidence CSV", "Back",
        ]
        for i, o in enumerate(opts, 1): console.print(f"[bold]{i}.[/bold] {o}")
        c = console.input("\nChoice: ").strip()
        ...
        elif c == "6": export_model_evidence_csv()
        elif c == "7": export_texture_evidence_csv()
        elif c == "8": return
...
def render_textures(rows, title):
    t = Table(title=title, header_style="bold cyan")
    t.add_column("#", justify="right"); t.add_column("Path"); t.add_column("Dim"); t.add_column("Format"); t.add_column("Flags"); t.add_column("Size", justify="right")
...
def model_explorer():
    console.clear(); header(); q = console.input("Model filename/path: ").strip().strip('"')
    db = Database(); m = db.model_by_query(q)
    if not m:
        db.close(); console.print("[red]Not found[/red]"); pause(); return
    links = db.links_for_model(m["path"]); evidence = db.evidence_for_model(m["path"], 50); db.close()
...
def texture_explorer():
    console.clear(); header(); q = console.input("Texture filename/path: ").strip().strip('"')
    db = Database(); tex = db.texture_by_query(q)
    if not tex:
        db.close(); console.print("[red]Not found[/red]"); pause(); return
    links = db.links_for_texture(tex["path"]); evidence = db.texture_evidence_for_texture(tex["path"], 50); db.close()
...
def duplicates_menu():
    console.clear(); header(); db = Database(); rows = db.duplicates("models", 50); db.close()
...
def families_menu():
    ...
    t = Table(title="Top Model Families", header_style="bold cyan")
    ...
    console.print(t)
    tt = Table(title="Top Texture Families", header_style="bold cyan")
    tt.add_column("#", justify="right"); tt.add_column("Name"); tt.add_column("Method"); tt.add_column("Members", justify="right")
...
def render_evidence(rows, title):
    t = Table(title=title, header_style="bold cyan")
    t.add_column("#", justify="right"); t.add_column("Score", justify="right"); t.add_column("Type"); t.add_column("A"); t.add_column("B"); t.add_column("Reasons")
...
def render_texture_evidence(rows, title):
    t = Table(title=title, header_style="bold cyan")
    t.add_column("#", justify="right"); t.add_column("Score", justify="right"); t.add_column("Type"); t.add_column("Texture A"); t.add_column("Texture B"); t.add_column("Reasons")
    for i, r in enumerate(rows, 1): t.add_row(str(i), str(r["overall_score"]), r["evidence_type"], r["path_a"], r["path_b"], (r["reasons"] or "")[:70])
    console.print(t)
...
def export_model_evidence_csv():
    out = timestamped_report_path("model_evidence_pairs")
    db = Database(); rows = db.top_evidence(100000); db.close()
...
# Compatibility alias in case an older menu path calls this name.
def export_evidence_csv():
    export_model_evidence_csv()


def export_texture_evidence_csv():
    out = timestamped_report_path("texture_evidence_pairs")
...
def stats():
    ...
    for k, v in [("Models", ms["total"]), ("Textures", ts["total"]), ("DDS Textures", ts["dds_count"] or 0), ("Model-Texture Links", rel["links"]), ("Model Families", rel["families"]), ("Texture Families", rel["texture_families"]), ("Model Evidence Pairs", rel["evidence_pairs"]), ("Texture Evidence Pairs", rel["texture_evidence_pairs"])]:
        t.add_row(k, f"{v:,}")
    t.add_row("Model Data", format_bytes(ms["total_size"])); t.add_row("Texture Data", format_bytes(ts["total_size"])); console.print(t)
