...
class Database:
    ...
    def columns(self, table): return {r["name"] for r in self.db.execute(f"PRAGMA table_info({table})")}
    def add_col(self, table, col, spec):
        if col not in self.columns(table): self.db.execute(f"ALTER TABLE {table} ADD COLUMN {col} {spec}")
    def set_setting(self, key, value): self.db.execute("INSERT OR REPLACE INTO settings(key,value) VALUES(?,?)", (key, value))
    ...
    def create_schema(self):
        ...
        """)
        # migrations
        self.add_col("models", "string_fingerprint", "TEXT DEFAULT ''")
        for col, spec in {
            "histogram_hash": "TEXT DEFAULT ''", "alpha_coverage": "REAL DEFAULT 0", "edge_density": "REAL DEFAULT 0",
            "brightness": "REAL DEFAULT 0", "saturation": "REAL DEFAULT 0", "is_grayscale": "INTEGER DEFAULT 0",
            "is_probable_normal": "INTEGER DEFAULT 0",
        }.items(): self.add_col("textures", col, spec)
        self.db.executescript("""
    ...
    def upsert_texture(self, row: dict[str, Any]):
        self.db.execute("""INSERT OR REPLACE INTO textures(path,root,relative_path,filename,folder,extension,size,mtime,sha256,md5,crc32,
            ...
            :width,:height,:mode,:has_alpha,:avg_r,:avg_g,:avg_b,:avg_a,:ahash,:histogram_hash,:alpha_coverage,:edge_density,:brightness,:saturation,:is_grayscale,:is_probable_normal,
            :analysis_status,:is_dds,:dds_width,:dds_height,:dds_mipmaps,:dds_fourcc,:dds_format,:dds_rgb_bits,:dds_has_alpha,:dds_is_cubemap,:dds_is_volume,:dds_estimated_vram,:dds_header_status,:last_scanned)""", row)
    ...
    def model_stats(self): return dict(self.db.execute("SELECT COUNT(*) total, COALESCE(SUM(size),0) total_size, COALESCE(AVG(size),0) avg_size, COUNT(DISTINCT sha256) unique_hashes FROM models").fetchone())
    def texture_stats(self): return dict(self.db.execute("SELECT COUNT(*) total, COALESCE(SUM(size),0) total_size, COALESCE(AVG(size),0) avg_size, COUNT(DISTINCT sha256) unique_hashes, SUM(CASE WHEN is_dds=1 THEN 1 ELSE 0 END) dds_count FROM textures").fetchone())
    ...
    def search_models(self, term, limit=100):
        like=f"%{term}%"; return self.db.execute("SELECT * FROM models WHERE filename LIKE ? OR folder LIKE ? OR relative_path LIKE ? OR sha256 LIKE ? ORDER BY folder,filename LIMIT ?", (like,like,like,like,limit)).fetchall()
    def search_textures(self, term, limit=100):
        like=f"%{term}%"; return self.db.execute("SELECT * FROM textures WHERE filename LIKE ? OR folder LIKE ? OR relative_path LIKE ? OR sha256 LIKE ? OR dds_format LIKE ? ORDER BY folder,filename LIMIT ?", (like,like,like,like,like,limit)).fetchall()
    def model_by_query(self, q): return self.db.execute("SELECT * FROM models WHERE path=? OR relative_path=? OR filename=? ORDER BY relative_path LIMIT 1", (q,q,q)).fetchone()
    def texture_by_query(self, q): return self.db.execute("SELECT * FROM textures WHERE path=? OR relative_path=? OR filename=? ORDER BY relative_path LIMIT 1", (q,q,q)).fetchone()
    def duplicates(self, table, limit=100): return self.db.execute(f"SELECT sha256, COUNT(*) count, SUM(size) total_size FROM {table} WHERE sha256!='' GROUP BY sha256 HAVING COUNT(*)>1 ORDER BY count DESC,total_size DESC LIMIT ?", (limit,)).fetchall()

    def clear_relationships(self): self.db.execute("DELETE FROM model_texture_links"); self.db.execute("DELETE FROM model_texture_status"); self.commit()
    def all_models(self): return self.db.execute("SELECT * FROM models ORDER BY folder,filename").fetchall()
    def textures_for_pid(self, folder, pid, limit=250): return self.db.execute("SELECT * FROM textures WHERE folder=? AND filename LIKE ? ORDER BY filename LIMIT ?", (folder, f"{pid}_%", limit)).fetchall()
    def textures_for_name(self, folder, base, limit=250): return self.db.execute("SELECT * FROM textures WHERE folder=? AND (filename LIKE ? OR filename LIKE ? OR filename LIKE ?) ORDER BY filename LIMIT ?", (folder, f"{base}_%", f"{base}.%", f"%{base}%", limit)).fetchall()
    def add_link(self, model_path, texture_path, score, method, reason): self.db.execute("INSERT OR REPLACE INTO model_texture_links(model_path,texture_path,score,method,reason,created) VALUES(?,?,?,?,?,?)", (model_path,texture_path,score,method,reason,time.time()))
    def set_model_texture_status(self, model_path, status, link_count, note): self.db.execute("INSERT OR REPLACE INTO model_texture_status(model_path,status,link_count,note,updated) VALUES(?,?,?,?,?)", (model_path,status,link_count,note,time.time()))
    def links_for_model(self, model_path, limit=100): return self.db.execute("""SELECT l.*, t.relative_path texture_relative_path, t.dds_format, t.width, t.height, t.dds_width, t.dds_height, t.sha256 texture_sha256, t.ahash FROM model_texture_links l JOIN textures t ON t.path=l.texture_path WHERE model_path=? ORDER BY score DESC LIMIT ?""", (model_path,limit)).fetchall()
    def links_for_texture(self, texture_path, limit=100): return self.db.execute("""SELECT l.*, m.relative_path model_relative_path, m.folder FROM model_texture_links l JOIN models m ON m.path=l.model_path WHERE texture_path=? ORDER BY score DESC LIMIT ?""", (texture_path,limit)).fetchall()

    def clear_families(self): self.db.execute("DELETE FROM model_family_members"); self.db.execute("DELETE FROM model_families"); self.commit()
    def create_family(self, name, method, confidence, members):
        ...
        for m in members: self.db.execute("INSERT OR REPLACE INTO model_family_members(family_id,model_path,confidence) VALUES(?,?,?)", (fid,m["path"],confidence))
    def family_groups(self, expr): return self.db.execute(f"SELECT {expr} key_value, COUNT(*) count FROM models WHERE {expr}!='' GROUP BY key_value HAVING COUNT(*)>1 ORDER BY count DESC").fetchall()
    def family_members_for(self, expr, key): return self.db.execute(f"SELECT * FROM models WHERE {expr}=? ORDER BY folder,filename", (key,)).fetchall()
    def families(self, limit=100): return self.db.execute("SELECT * FROM model_families ORDER BY member_count DESC,confidence DESC LIMIT ?", (limit,)).fetchall()
    def family_members(self, fid, limit=500): return self.db.execute("SELECT fm.*, m.* FROM model_family_members fm JOIN models m ON m.path=fm.model_path WHERE family_id=? ORDER BY m.folder,m.filename LIMIT ?", (fid,limit)).fetchall()

    def clear_texture_families(self): self.db.execute("DELETE FROM texture_family_members"); self.db.execute("DELETE FROM texture_families"); self.commit()
    def texture_family_groups(self, expr): return self.db.execute(f"SELECT {expr} key_value, COUNT(*) count FROM textures WHERE {expr}!='' GROUP BY key_value HAVING COUNT(*)>1 ORDER BY count DESC").fetchall()
    def texture_family_members_for(self, expr, key): return self.db.execute(f"SELECT * FROM textures WHERE {expr}=? ORDER BY folder,filename", (key,)).fetchall()
